<?php
    ...
    $html = <<<EOD
    <h1>
    Laporan Data Alat Masuk SI Inventori LAB.RPL
    </h1>
...
EOD;
    ...
    $table = '<p align="left"> Periode : '.date('d M Y',strtotime($tglAw)).' s/d '.date('d M Y',strtotime($tglAk)).'</p> <table border="1"  cellpadding="0" cellspacing="3" align="center" fontsize="12">';
        $table .= '
            ...
            <th width="45">No</th>
            <th colspan="1" width="80">Kode Barang</th>
            <th colspan="1" width="100">Nama Barang</th>
            <th colspan="1" width="100">Spesifikasi</th>
            <th colspan="1" width="80">Kondisi</th>
            <th colspan="1" width="70">Jumlah</th>
            <th colspan="1" width="80">Sumber Masuk</th>
            <th colspan="1" width="80">Tanggal Masuk</th>
            </tr>
            ';
        $no=1;
        ...
        $table .='
            <tr>
            <td width="45">'.$no.'</td>
            <td colspan="1" width="80">'.$keluar->kode_barang.'</td>
            <td colspan="1" width="100">'.$keluar->nama_barang.'</td>
            <td colspan="1" width="100">'.$keluar->spesifikasi.'</td>
            <th colspan="1" width="80">'.$keluar->kondisi.'</th>
            <td colspan="1" width="70">'.$keluar->jumlah.'</td>
            <td colspan="1" width="80">'.$keluar->sumber_barang.'</td>
            <td colspan="1" width="80">'.$keluar->tanggal_masuk.'</td>
            </tr>';
        $no++;
        }
    ...
    $pdf->Output('Laporan_BarangMasuk.pdf', 'I'); 
?>
